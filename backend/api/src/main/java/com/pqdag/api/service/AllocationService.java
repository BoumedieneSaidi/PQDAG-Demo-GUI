...
public class AllocationService {
    ...
    public AllocationResponse distributeFragments(AllocationRequest request) throws Exception {
        ...
        // Run distribute_fragments.py
        ProcessBuilder pb = new ProcessBuilder(
            "python3",
            workspaceRoot + "/backend/allocation/distribute_fragments.py",
            "--config_file",
            workspaceRoot + "/backend/allocation/config_runtime.yaml"
        );
        pb.directory(new File(workspaceRoot + "/backend/allocation"));
    ...
    private void runStatistics(String datasetName) throws Exception {
        ...
        // Docker command for running stat_MPI.py
        ProcessBuilder pb = new ProcessBuilder(
            "docker", "run", "--rm",
            "-v", workspaceRoot + "/storage:/app/storage",
            "-v", workspaceRoot + "/backend/allocation:/app/allocation",
            dockerImage,
            "bash", "-c",
