...
def send_files_to_workers(worker_id_to_ip, temp_dir, destination_path, ssh_user):
    """
    Distribute fragment archives to workers using SSH aliases configured for jump host.
    The SSH config (pqdag-worker-X) handles the ProxyJump automatically.
    """
    for worker_id, ip_address in worker_id_to_ip.items():
        tar_file = os.path.join(temp_dir, f'worker_{worker_id}.tar.gz')
        if os.path.exists(tar_file):
            # Use SSH alias instead of direct IP (handles jump host automatically)
            ssh_alias = f"pqdag-worker-{worker_id}"
            
            # Ensure destination directory exists on worker
            print(f"Creating destination directory on worker {worker_id} ({ip_address})...")
            ssh_command = f"ssh {ssh_alias} 'mkdir -p {destination_path}'"
            ...
            scp_command = f'scp "{tar_file}" {ssh_alias}:{destination_path}'
            ...
            ssh_command = f"ssh {ssh_alias} 'cd {destination_path} && tar -xzf {os.path.basename(tar_file)} && rm {os.path.basename(tar_file)}'"
            result = os.system(ssh_command)
            if result != 0:
...
def send_files_to_master(master_ip, common_files, fragment_files_dir, destination_path, ssh_user, affectation_file):
    """
    Send common files and global affectation to master using SSH alias.
    The SSH config (pqdag-master) handles the ProxyJump automatically.
    """
    # Use SSH alias instead of direct IP
    ssh_alias = "pqdag-master"
    ...
    for common_file in common_files:
        ...
        if os.path.exists(src_file):
            ...
            scp_command = f'scp "{src_file}" {ssh_alias}:{destination_path}/'
    ...
    if os.path.exists(affectation_file):
        ...
        scp_command = f'scp "{affectation_file}" {ssh_alias}:{destination_path}/{dst_affectation_file}'
        os.system(scp_command)
    else:
...
def ensure_storage_folder(worker_id_to_ip, storage_folder, ssh_user):
    """
    Ensure storage folder exists on all workers using SSH aliases.
    """
    for worker_id, ip_address in worker_id_to_ip.items():
        # Use SSH alias instead of direct IP
        ssh_alias = f"pqdag-worker-{worker_id}"
        ...
        ssh_command = f"ssh {ssh_alias} 'mkdir -p {storage_folder}'"
        os.system(ssh_command)

def run_loader_script(worker_id_to_ip, destination_path, storage_folder, ssh_user):
    """
    Run the fragments_loader.py script on all workers using SSH aliases.
    """
    for worker_id, ip_address in worker_id_to_ip.items():
        # Use SSH alias instead of direct IP
        ssh_alias = f"pqdag-worker-{worker_id}"
        ...
        ssh_command = f"ssh {ssh_alias} 'bash -l -c \"{command}\"'"
        os.system(ssh_command)

def cleanup_destination(worker_id_to_ip, master_ip, destination_path, ssh_user):
    """
    Deletes the content of the destination path on all workers and master using SSH aliases.
    """
    for worker_id, ip_address in worker_id_to_ip.items():
        # Use SSH alias instead of direct IP
        ssh_alias = f"pqdag-worker-{worker_id}"
        ...
        ssh_command = f"ssh {ssh_alias} 'rm -rf {destination_path}/*'"
        os.system(ssh_command)
    
    # Clean up on the master as well (use alias)
    print(f"Cleaning up destination path on master ({master_ip})...")
    ssh_command = f"ssh pqdag-master 'rm -rf {destination_path}/*'"
    os.system(ssh_command)
