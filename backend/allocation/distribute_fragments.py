...
def send_files_to_workers(worker_id_to_ip, temp_dir, destination_path, ssh_user):
    """
    Distribute fragment archives to workers using direct IP addresses with SSH.
    """
    ssh_key = "/home/pqdag/.ssh/pqdag"
    ...
    for worker_id, ip_address in worker_id_to_ip.items():
        ...
        if os.path.exists(tar_file):
            ...
            ssh_target = f"{ssh_user}@{ip_address}"
            
            # Ensure destination directory exists on worker
            print(f"Creating destination directory on worker {worker_id} ({ip_address})...")
            ssh_command = f"ssh -i {ssh_key} {ssh_opts} {ssh_target} 'mkdir -p {destination_path}'"
            result = os.system(ssh_command)
            if result != 0:
                print(f"Warning: Failed to create directory on worker {worker_id}")
                continue

            # Use scp to copy the tar file to the worker
            print(f"Sending tar file to worker {worker_id} ({ip_address})...")
            scp_command = f'scp -i {ssh_key} {ssh_opts} "{tar_file}" {ssh_target}:{destination_path}'
            result = os.system(scp_command)
            if result != 0:
                print(f"Warning: Failed to send tar file to worker {worker_id}")
                continue

            # Extract the tar file on the worker
            print(f"Extracting tar file on worker {worker_id}...")
            ssh_command = f"ssh -i {ssh_key} {ssh_opts} {ssh_target} 'cd {destination_path} && tar -xzf {os.path.basename(tar_file)} && rm {os.path.basename(tar_file)}'"
            result = os.system(ssh_command)
            if result != 0:
                print(f"Warning: Failed to extract tar file on worker {worker_id}")
        else:
...
def send_files_to_master(master_ip, common_files, fragment_files_dir, destination_path, ssh_user, affectation_file):
    """
    Send common files and global affectation to master using direct IP.
    """
    ssh_key = "/home/pqdag/.ssh/pqdag"
    ssh_opts = "-o StrictHostKeyChecking=no -o ConnectTimeout=10"
    ssh_target = f"{ssh_user}@{master_ip}"
    
    # Ensure destination directory exists on master
    print(f"Creating destination directory on master ({master_ip})...")
    ssh_command = f"ssh -i {ssh_key} {ssh_opts} {ssh_target} 'mkdir -p {destination_path}'"
    ...
    for common_file in common_files:
        src_file = os.path.join(fragment_files_dir, common_file)
        if os.path.exists(src_file):
            print(f"Sending {common_file} to master...")
            scp_command = f'scp -i {ssh_key} {ssh_opts} "{src_file}" {ssh_target}:{destination_path}/'
    ...
    if os.path.exists(affectation_file):
        print(f"Sending global affectation file to master...")
        scp_command = f'scp -i {ssh_key} {ssh_opts} "{affectation_file}" {ssh_target}:{destination_path}/{dst_affectation_file}'
        os.system(scp_command)
    else:
...
def ensure_storage_folder(worker_id_to_ip, storage_folder, ssh_user):
    """
    Ensure storage folder exists on all workers using direct IP addresses.
    """
    ssh_key = "/home/pqdag/.ssh/pqdag"
    ssh_opts = "-o StrictHostKeyChecking=no -o ConnectTimeout=10"
    ...
    for worker_id, ip_address in worker_id_to_ip.items():
        ...
        print(f"Ensuring storage folder exists on worker {worker_id} ({ip_address})...")
        ssh_command = f"ssh -i {ssh_key} {ssh_opts} {ssh_target} 'mkdir -p {storage_folder}'"
        os.system(ssh_command)

def run_loader_script(worker_id_to_ip, destination_path, storage_folder, ssh_user):
    """
    Run the fragments_loader.py script on all workers using direct IP addresses.
    """
    ssh_key = "/home/pqdag/.ssh/pqdag"
    ssh_opts = "-o StrictHostKeyChecking=no -o ConnectTimeout=10"
    ...
    for worker_id, ip_address in worker_id_to_ip.items():
        ...
        print(f"Running fragments_loader.py on worker {worker_id} ({ip_address})...")
        ...
        ssh_command = f"ssh -i {ssh_key} {ssh_opts} {ssh_target} 'bash -l -c \"{command}\"'"
        os.system(ssh_command)

def cleanup_destination(worker_id_to_ip, master_ip, destination_path, ssh_user):
    """
    Deletes the content of the destination path on all workers and master using direct IP addresses.
    """
    ssh_key = "/home/pqdag/.ssh/pqdag"
    ssh_opts = "-o StrictHostKeyChecking=no -o ConnectTimeout=10"
    ...
    for worker_id, ip_address in worker_id_to_ip.items():
        ...
        print(f"Cleaning up destination path on worker {worker_id} ({ip_address})...")
        ssh_command = f"ssh -i {ssh_key} {ssh_opts} {ssh_target} 'rm -rf {destination_path}/*'"
        os.system(ssh_command)
    
    # Clean up on the master as well
    print(f"Cleaning up destination path on master ({master_ip})...")
    ssh_target_master = f"{ssh_user}@{master_ip}"
    ssh_command = f"ssh -i {ssh_key} {ssh_opts} {ssh_target_master} 'rm -rf {destination_path}/*'"
    os.system(ssh_command)
