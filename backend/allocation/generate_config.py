...
def generate_config(workspace_root: str, dataset_name: str, output_path: str = None) -> dict:
    ...
    """
    # Read template
    # In Docker container, allocation code is mounted at /app/allocation/
    # Check if running in container (allocation dir directly under workspace_root)
    template_path_docker = Path(workspace_root) / 'allocation' / 'config.yaml'
    template_path_local = Path(workspace_root) / 'backend' / 'allocation' / 'config.yaml'
    
    if template_path_docker.exists():
        template_path = template_path_docker
        config_runtime_path = Path(workspace_root) / 'allocation' / 'config_runtime.yaml'
    elif template_path_local.exists():
        template_path = template_path_local
        config_runtime_path = Path(workspace_root) / 'backend' / 'allocation' / 'config_runtime.yaml'
    else:
        raise FileNotFoundError(f"Config template not found: {template_path_docker} or {template_path_local}")
    ...
    if output_path is None:
        output_path = config_runtime_path
